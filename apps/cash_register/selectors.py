...
def get_cash_sessions_for_history(*, business, store, filters=None):
    filters = filters or {}
    queryset = CashSession.objects.filter(
        business=business, store=store
    ).select_related("cash_register", "opened_by", "closed_by")
    if filters.get("cash_register"):
        queryset = queryset.filter(cash_register=filters["cash_register"])
    if filters.get("user"):
        user = filters["user"]
        queryset = queryset.filter(Q(opened_by=user) | Q(closed_by=user))
    if filters.get("date_from"):
        queryset = queryset.filter(opened_at__date__gte=filters["date_from"])
    if filters.get("date_to"):
        queryset = queryset.filter(opened_at__date__lte=filters["date_to"])
    return queryset.order_by("-opened_at", "-pk")
