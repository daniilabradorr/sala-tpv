...
class Customer(TimeStampedModel):
    ...
    El cliente pertenece al negocio completo, no a una tienda concreta.
    Puede comprar en cualquiera de las tiendas del mismo negocio.

    Los datos fiscales pueden completarse posteriormente, pero serán
    necesarios cuando se emita una factura completa F1 o F3.
    """
    ...
    def fiscal_name(self):
        """Nombre que se utilizará como nombre fiscal del cliente."""

        return self.legal_name or self.name

    @property
    def has_national_tax_data(self):
        """Indica si tiene identificación fiscal nacional."""

        return bool(self.tax_identifier)

    @property
    def has_foreign_tax_data(self):
        """Indica si tiene identificación fiscal extranjera completa."""

        return bool(self.foreign_id_type and self.foreign_id)

    def _normalize_fields(self):
        """Normaliza textos antes de validar y guardar."""
    ...
    def clean(self):
        """Protege la coherencia básica del cliente."""
        ...
        has_foreign_type = bool(self.foreign_id_type)
        has_foreign_number = bool(self.foreign_id)

        if has_foreign_type != has_foreign_number:
            message = (
...
class CustomerAccount(TimeStampedModel):
    ...
    CustomerAccount representa la fotografía actual del saldo.

    El balance no debe modificarse desde views, forms ni admin.
    Debe modificarse únicamente mediante CustomerAccountService.
    """
    ...
    balance = models.DecimalField(
        ...
        editable=False,
        help_text=(
            "Saldo actual. Positivo significa deuda del cliente; "
            "negativo significa saldo a favor."
        ),
    )
    ...
    def available_credit(self):
        """Crédito que todavía puede utilizar el cliente."""

        current_debt = max(self.balance, Decimal("0.00"))
        return self.credit_limit - current_debt

    def clean(self):
        """Protege el aislamiento multiempresa y el límite de crédito."""
...
class CustomerAccountEntry(TimeStampedModel):
    ...
    CustomerAccount guarda el saldo actual.

    CustomerAccountEntry guarda cada cambio individual y el saldo
    resultante después de dicho cambio.

    Convención de signos:
    - charge: importe positivo.
    - payment: importe negativo.
    - refund: importe negativo.
    - adjustment: importe positivo o negativo, pero nunca cero.

    Las relaciones con Sale y Payment se añadirán cuando existan
    los modelos reales de sales y payments.
    """
    ...
    def clean(self):
        """Protege la coherencia del movimiento."""
        ...
        if self.created_by_id and self.business_id:
            user_business_id = getattr(
                self.created_by,
                "business_id",
                None,
            )

            if user_business_id is not None and user_business_id != self.business_id:
