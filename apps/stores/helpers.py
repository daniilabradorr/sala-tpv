...
def belongs_to_business(user, business):
    ...
    Los superusuarios pueden trabajar con cualquier negocio, pero
    deberán utilizar un contexto de negocio explícito en las vistas
    que lo necesiten.
    """
...
def can_access_store(user, store):
    ...
    Una tienda inactiva puede seguir consultándose para:
    - ver información histórica;
    - administrarla;
    - reactivarla.

    La tienda inactiva se bloqueará únicamente para operaciones como
    vender, abrir caja o cerrar caja.
    """
    if not is_authenticated_user(user):
        return False
...
def can_sell_in_store(user, store):
    ...
    No se puede vender en una tienda inactiva, aunque el usuario
    siga teniendo acceso administrativo a ella.
    """
...
def can_perform_sensitive_action(user):
    ...
    El PIN, cuando corresponda, se valida en una capa separada.
    """
    if not is_authenticated_user(user):
        return False
