...
def set_default_store(*, business, store):
    ...
    El modelo Store se encargará de quitar la marca de predeterminada
    a la tienda anterior.

    Esta operación es idempotente:
    si la tienda ya es predeterminada, no hace cambios innecesarios.
    """
...
def activate_store(*, business, store):
    ...
    Si el negocio no tiene ninguna tienda predeterminada, el save()
    del modelo convierte esta tienda en la predeterminada.

    Es idempotente:
    activar una tienda ya activa no produce un error.
    """
    locked_store = _get_locked_store(
        business=business,
        store=store,
...
def deactivate_store(*, business, store):
    ...
    Si era la tienda predeterminada, el modelo intenta seleccionar
    como predeterminada otra tienda activa del mismo negocio.

    Es idempotente:
    desactivar una tienda ya inactiva no produce un error.
    """
    locked_store = _get_locked_store(
        business=business,
        store=store,
