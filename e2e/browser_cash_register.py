...
class CashRegisterBrowserTests(StaticLiveServerTestCase):
    ...
    def test_register_open_tabs_and_responsive_layouts(self):
        ...
        with sync_playwright() as playwright:
            ...
            page.get_by_label("Caja", exact=True).select_option(str(closed.pk))
            ...
            page.get_by_label("Caja", exact=True).select_option("")
            page.get_by_label("Usuario", exact=True).select_option(str(user.pk))
            page.get_by_role("button", name="Filtrar").click()
            expect(results).to_contain_text("Caja A")
            expect(results).to_contain_text("Caja B")
            ...
            page.go_back()
            self.assertIn(f"cash_register={closed.pk}", page.url)
            expect(page.get_by_label("Caja", exact=True)).to_have_value(str(closed.pk))
            expect(page.get_by_label("Usuario", exact=True)).to_have_value("")
            expect(results).to_contain_text("Caja A")
            expect(results).not_to_contain_text("Caja B")
            self.assertEqual(page.locator("#cash-history-results").count(), 1)
            page.go_forward()
            self.assertIn(f"user={user.pk}", page.url)
            expect(page.get_by_label("Caja", exact=True)).to_have_value("")
            expect(page.get_by_label("Usuario", exact=True)).to_have_value(str(user.pk))
            expect(results).to_contain_text("Caja A")
            expect(results).to_contain_text("Caja B")
