            ...
            try:
                for index, viewport in enumerate(
                    ...
                    with self.subTest(viewport=viewport):
                        ...
                        expect(page.get_by_text("Paso 1 de 4").first).to_be_visible()

                        page.get_by_label("Razón social").fill(f"Taller E2E {index}")
                        page.get_by_label("NIF/CIF").fill(f"B1234567{index}")
                        page.get_by_label("Nombre comercial").fill(f"Taller {index}")
                        page.locator("#id_phone").fill("910000000")
                        page.locator("#id_email").fill(f"negocio{index}@example.com")
                        ...
                        page.get_by_label("Usar la misma dirección del negocio").check()
                        expect(custom).to_be_hidden()
                        page.get_by_role("button", name="Continuar").click()
                        ...
                        expect(page.get_by_text(f"Taller E2E {index}")).to_be_visible()
                        review = page.locator("[data-step='4']")
                        expect(review).not_to_contain_text("A-secure-password-2026!")
                        expect(review).not_to_contain_text("2468")
                        page.get_by_role("button", name="Crear mi Netxodo").click()
                        expect(
                            page.get_by_role("heading", name="Tu Netxodo está listo")
                        ).to_be_visible()
                        expect(page.locator("[data-app-shell]")).to_have_count(0)
                        ...
                        expect(
                            page.get_by_role("link", name="Configura el stock inicial")
                        ).to_be_visible()
                        expect(
